...
func AuditNode(w *model.World, node *model.Node) *model.AuditReport {
	...
	if len(volumes) > 0 {
		disks := report.GetOrCreateTable("Device", "Mount points", "Used by", "Latency", "I/O Load", "Space").Group("Disks", 4)
		ioLatencyChart := report.GetOrCreateChartGroup("Average I/O latency <selector>, seconds", nil).Group("Disks", 4)
		ioLoadChart := report.GetOrCreateChartGroup("I/O load (total latency) <selector>, seconds/second", nil).Group("Disks", 4)
		iopsChart := report.GetOrCreateChartGroup("IOPS <selector>", nil).Group("Disks", 4)
		bandwidthChart := report.GetOrCreateChartGroup("Bandwidth <selector>, bytes/second", nil).Group("Disks", 4)
		spaceChart := report.GetOrCreateChartGroup("Disk space <selector>, bytes", nil).Group("Disks", 4)
	...
	for _, gpu := range node.GPUs {
		gpus := report.GetOrCreateTable("GPU UUID", "Name", "Driver", "vRAM").Group("GPUs", 5)
