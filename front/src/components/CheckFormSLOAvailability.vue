<template>
    <v-form v-if="form" v-model="valid">
        <div v-for="c in form.configs">
            ...
            <MetricSelector v-model="c.total_requests_query" :rules="[$validators.notEmpty]" wrap="sum( rate( <input> [..]) )" class="mb-3"/>

            Failed requests query:
            <MetricSelector v-model="c.failed_requests_query" :rules="[$validators.notEmpty]" wrap="sum( rate( <input> [..]) )" class="mb-3"/>
