...
func BuiltinAlertingRules() []AlertingRule {
	return []AlertingRule{
		...
		{
			Id:   "postgres-checkpoints",
			Name: "Postgres checkpoints",
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.PostgresCheckpoint.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      WARNING,
			For:           5 * timeseries.Minute,
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "PostgreSQL is not completing checkpoints on schedule. The checkpointer may be stuck, risking a long crash recovery.",
			},
			Enabled: true,
			Builtin: true,
		},
		{
			Id:   "postgres-wal-archiving",
			Name: "Postgres WAL archiving",
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.PostgresWalArchiving.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      WARNING,
			For:           5 * timeseries.Minute,
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "PostgreSQL is failing to archive WAL. WAL files cannot be recycled and will accumulate until the disk fills.",
			},
			Enabled: true,
			Builtin: true,
		},
		{
			Id:   "postgres-wraparound",
			Name: "Postgres transaction ID wraparound",
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.PostgresWraparound.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      CRITICAL,
			For:           5 * timeseries.Minute,
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "PostgreSQL is approaching transaction ID wraparound. If the age reaches the limit the database will stop accepting writes. Vacuum freezing is likely blocked by a held xmin horizon (a replication slot, long-running or prepared transaction).",
			},
			Enabled: true,
			Builtin: true,
		},
		{
			...
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.PostgresBloat.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      WARNING,
			...
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "A large share of a PostgreSQL database is estimated to be wasted space (bloat). Reclaim it with pg_repack (online) or VACUUM FULL/REINDEX.",
			},
			Enabled: true,
			Builtin: true,
		},
		{
			Id:   "postgres-autovacuum",
			Name: "Postgres autovacuum",
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.PostgresAutovacuum.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      WARNING,
			For:           10 * timeseries.Minute,
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "Dead rows are accumulating in a PostgreSQL database faster than autovacuum can reclaim them, risking bloat and degraded performance. Check for snapshot-pinning transactions/slots and autovacuum tuning.",
			},
			Enabled: true,
			Builtin: true,
		},
		{
			Id:   "postgres-stale-statistics",
			Name: "Postgres stale statistics",
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.PostgresStaleStatistics.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      WARNING,
			For:           10 * timeseries.Minute,
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "A large table has changed far past its autoanalyze threshold, so the query planner is working from stale row estimates and may pick bad plans (like a sequential scan instead of an index). Run ANALYZE and check why autoanalyze fell behind.",
			},
			Enabled: true,
			Builtin: true,
		},
		{
			...
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.PostgresBackups.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      WARNING,
			For:           10 * timeseries.Minute,
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "PostgreSQL backups are stale or failing: either no successful backup completed within the expected window, the most recent backup failed, or continuous WAL archiving has stopped. Without healthy backups and archiving, point-in-time recovery is not possible. Check the backup target and the database logs.",
			},
			Enabled: true,
			Builtin: true,
		},
		...
		{
			Id:   "mysql-connection-errors",
			Name: "MySQL connection errors",
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.MysqlConnectionErrors.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      WARNING,
			For:           2 * timeseries.Minute,
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "Clients are failing to connect to MySQL (aborted connection attempts) - typically bad credentials, TLS errors, or handshake timeouts.",
			},
			Enabled: true,
			Builtin: true,
		},
		{
			Id:   "mysql-galera-replication",
			Name: "MySQL Galera replication",
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.MysqlGaleraReplication.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      WARNING,
			For:           2 * timeseries.Minute,
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "A Galera node's replication is unhealthy: writes throttled by flow control, a node falling behind on apply, or certification conflicts from concurrent writes to the same rows.",
			},
			Enabled: true,
			Builtin: true,
		},
		{
			...
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.MysqlGroupReplication.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      WARNING,
			For:           2 * timeseries.Minute,
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "A MySQL Group Replication member is unhealthy: not ONLINE, the group has lost redundancy, or a member's apply/certification queue is backing up and throttling group-wide writes.",
			},
			Enabled: true,
			Builtin: true,
		},
		{
			...
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.MysqlLatency.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      WARNING,
			For:           5 * timeseries.Minute,
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "MySQL query latency is elevated. Dependent applications may be slow.",
			},
			Enabled: true,
			Builtin: true,
		},
		{
			...
			Source: AlertSource{
				Type:  AlertSourceTypeCheck,
				Check: &CheckSource{CheckId: Checks.MysqlBackups.Id},
			},
			Selector:      AppSelector{Type: AppSelectorTypeAll},
			Severity:      WARNING,
			For:           10 * timeseries.Minute,
			KeepFiringFor: 5 * timeseries.Minute,
			Templates: AlertTemplates{
				Description: "MySQL backups are stale or failing: either no successful backup completed within the expected window, the most recent backup failed, or the scheduled backup is overdue. Check the backup storage and the backup job logs.",
			},
			Enabled: true,
			Builtin: true,
		},
