...
| --auth-handoff-secret                | AUTH_HANDOFF_SECRET                |               | Shared secret for trusted one-time session handoff (`POST /api/auth/handoff`). Used by Kubero server-to-server calls (`X-Handoff-Secret` or `Authorization: Bearer`).            |
| --license-key                        | LICENSE_KEY                        |               | License key for Coroot Enterprise Edition.                                                                                                                                      |
| --rca-provider                       | RCA_PROVIDER                       | cloud         | Root cause analysis backend: `cloud` (Coroot Cloud) or `local` (self-hosted OpenAI-compatible LLM).                                                                             |
| --rca-llm-base-url                   | RCA_LLM_BASE_URL                   |               | Base URL of an OpenAI-compatible API, e.g. `https://openrouter.ai/api/v1` or an internal gateway. Required when the provider is `local`.                                        |
| --rca-llm-api-key                    | RCA_LLM_API_KEY                    |               | API key for the OpenAI-compatible API.                                                                                                                                          |
| --rca-llm-model                      | RCA_LLM_MODEL                      |               | Model used for root cause analysis, e.g. `moonshotai/kimi-k2.6`. Required when the provider is `local`.                                                                         |
| --rca-system-prompt                  | RCA_SYSTEM_PROMPT                  |               | Override the built-in root cause analysis system prompt.                                                                                                                        |
| --rca-auto-investigate               | RCA_AUTO_INVESTIGATE               | false         | Automatically investigate incidents as they are detected. When disabled, analysis runs only when requested from the incident page.                                              |
| --rca-timeout                        | RCA_TIMEOUT                        | 5m            | Timeout for a single root cause analysis.                                                                                                                                       |
