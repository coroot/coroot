			...
			for hash, lp := range logMsgs.Patterns {
				...
				if blockingAlert := w.findBlockingAlert(string(rule.Id), fingerprint, appId, lp, alertByFingerprint, alertsByApp); blockingAlert != nil {
					...
					if !blockingAlert.Suppressed && blockingAlert.ManuallyResolvedAt == 0 {
						...
						if lp.Sample != "" {
							updatedDetails = append(updatedDetails, model.AlertDetail{Name: "Sample", Value: utils.Truncate(lp.Sample, alertDetailValueMaxLen), Code: true})
						}
				...
				if lp.Sample != "" {
					logDetails = append(logDetails, model.AlertDetail{Name: "Sample", Value: utils.Truncate(lp.Sample, alertDetailValueMaxLen), Code: true})
				}
				if aiExplanation != "" {
...
func (w *Alerts) evaluateKubernetesEventsAlerts(project *db.Project, rule *model.AlertingRule, world *model.World, from, to timeseries.Time, now timeseries.Time) {
	...
	for _, g := range groups {
		...
		if g.events[0].Body != "" {
			details = append(details, model.AlertDetail{Name: "Event message", Value: utils.Truncate(g.events[0].Body, alertDetailValueMaxLen), Code: true})
		}
	...
	for _, app := range apps {
		...
		for sev, logMsgs := range app.LogMessages {
			...
			for hash, lp := range logMsgs.Patterns {
				...
				if lp.Sample != "" {
					initDetails = append(initDetails, model.AlertDetail{Name: "Sample", Value: utils.Truncate(lp.Sample, alertDetailValueMaxLen), Code: true})
				}
				alert := &model.Alert{
					Id:                  utils.NanoId(12),
					Fingerprint:         fingerprint,
